      ...
      <div className="flex flex-col lg:flex-row gap-4 mt-6">
        ...
        <div className='w-full lg:w-8/12 border rounded-3xl px-5 py-6 border-restro-green'>
          ...
          <div className="flex flex-col gap-4 mt-4">
            {subscriptionHistory && subscriptionHistory?.filter(item => item.status !== "updated")?.map((item, i)=>{

              const {
                id, tenant_id, created_at, starts_on, expires_on, status, plan_title, is_trial, amount, currency, symbol, frequency
              } = item;
                ...
                <div key={i} className="flex items-center gap-4 w-full">
                  ...
                  <div className="flex-grow">
                    <div className="flex justify-between w-full gap-1">
                      <p className="text-lg">
                        <span className="font-bold">{plan_title ? `${symbol}${amount}/${frequency}` : t("superadmin_tenant_subscription_history.subscription_price", { price: subscriptionPrice })}</span>
                        <span className='text-sm'>{" "}{ plan_title && `(${plan_title})`}{" "}</span>
                        {/* {is_trial === 1 ? (<span className='text-xs font-semibold  px-2 rounded-full min-w-20 sm:w-18 text-center bg-green-100 text-green-500 border border-green-500'>Trial Plan</span>) : ""} */}
                      </p>
                      <div
                        className={`text-xs font-semibold py-1 px-3 rounded-full min-w-20 sm:w-18 text-center ${
