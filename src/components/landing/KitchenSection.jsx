...
import { LANDING_KITCHEN } from "./constants";
import { useMotionConfig } from "./useReducedMotionSafe";

export default function KitchenSection() {
  const { fadeUp, reduced } = useMotionConfig();
  const { title, description, image, imageAlt, cta, highlights } = LANDING_KITCHEN;

  return (
    <section id="cocina" className="py-10 px-4 bg-restro-gray/40">
          ...
          <motion.div {...fadeUp}>
            <div className="flex items-center gap-3 mb-6">
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-restro-green-10 border border-restro-border-green">
                <IconChefHat size={22} className="text-restro-green" stroke={1.75} />
              </div>
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-restro-green-10 border border-restro-border-green">
                <IconBell size={22} className="text-restro-green" stroke={1.75} />
              </div>
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-restro-green-10 border border-restro-border-green">
                <IconChecks size={22} className="text-restro-green" stroke={1.75} />
              </div>
            </div>
