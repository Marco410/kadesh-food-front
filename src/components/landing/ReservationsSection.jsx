...
import { LANDING_RESERVATIONS } from "./constants";
import { useMotionConfig } from "./useReducedMotionSafe";

export default function ReservationsSection() {
  const { fadeUp, reduced } = useMotionConfig();
  const { title, description, image, imageAlt, cta, highlights } = LANDING_RESERVATIONS;

  return (
    <section id="reservaciones" className="py-10 px-4 bg-restro-gray/40">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          <motion.div {...fadeUp}>
            <div className="flex items-center gap-3 mb-6">
              ...
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-restro-green-10 border border-restro-border-green">
                <IconArmchair size={22} className="text-restro-green" stroke={1.75} />
              </div>
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-restro-green-10 border border-restro-border-green">
                <IconSearch size={22} className="text-restro-green" stroke={1.75} />
              </div>
            </div>
