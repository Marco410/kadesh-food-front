...
import { LANDING_POS } from "./constants";
import { useMotionConfig } from "./useReducedMotionSafe";

export default function POSSection() {
  const { fadeUp, reduced } = useMotionConfig();
  const { title, description, image, imageAlt, cta, highlights } = LANDING_POS;

  return (
    <section id="pos" className="py-10 px-4 bg-restro-green-light/40">
          ...
          <motion.div {...fadeUp}>
            <div className="flex items-center gap-3 mb-6">
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-restro-green-10 border border-restro-border-green">
                <IconCashRegister size={22} className="text-restro-green" stroke={1.75} />
              </div>
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-restro-green-10 border border-restro-border-green">
